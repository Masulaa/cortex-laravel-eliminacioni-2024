            ...
            @if ($post->picture)
                <a href="/post/{{ $post->slug }}">
                    <div class="card-header">
                        <img class="w-full h-56 object-cover" src="{{ $post->picture }}"
            ...
            <div class="flex flex-col justify-center items-start px-5 min-h-64">
                ...
                @foreach ($post->categories as $category)
                    <a href="{{ route('category.posts', $category->id) }}">
                        <span
                            class="text-[11px] m-0 rounded-md text-white py-1 px-2 cursor-pointer bg-[#47bcd4]">{{ $category->name }}</span></a>
                @endforeach
